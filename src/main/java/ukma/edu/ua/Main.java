...
import ukma.edu.ua.network.ClientUDP;
import ukma.edu.ua.network.ServerUDP;
...
//        });
        ...
        serverExecutor.submit(() -> {
            try {
                final ServerUDP serverUDP = new ServerUDP();
                serverUDP.receivePackets(receiver);
            } catch (IOException e) {
                throw new RuntimeException("UDP server failed", e);
            }
        });

        ClientTCP client1 = new ClientTCP();
        ClientUDP updClient = new ClientUDP();

        Message message1 = new Message(1, 1, "Client".getBytes(UTF_8));
        Packet packet1 = new Packet((byte) 1, 1, message1);


        updClient.send(packet1);
...
//        sendMessages(client3);
    ...
    private static void sendMessages(ClientTCP client) {
        ...
        Packet packet3 = new Packet((byte) 1, 3, message3);
